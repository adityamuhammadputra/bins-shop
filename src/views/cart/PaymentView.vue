        ...
        <div class="container">
            <div class="row">
                <div class="col-12" v-if="!isMobile()">
                    <h5 class="title mb-3">
                        <router-link to="/cart" class="image">
                ...
                <div class="col-md-8 col-12">
                    <div class="card border-none-mobile" v-if="user">
                        <div class="card-body p-0-mobile">
                            <span v-if="products.length > 0"><b>Detail Barang</b> </span>
                            <div class="comment-area-wrapper mt-2">
                                <div v-if="products.length > 0">
                                    <div v-for="(product, indexCart) in products" v-bind:key="product.id">
                                        <div v-if="product.status == true" style="border-bottom: 1px solid #f3f3f3;padding-bottom: 10px;margin-bottom: 10px;">
                                            <div class="single-comment-wrap">
                                                <a class="author-thumb" href="#" style="width: 55px;height: 55px;">
                                                    <router-link :to="'/product/'+products[indexCart].product.slug+ '?back=cart'" class="image">
                                                        <img :src="product.product.file" alt="Author">
                                                    </router-link>
                                                </a>
                                                <div class="comments-info">
                                                    <p class="mb-0" style="font-size: 14px;">
                                                        <router-link :to="'/product/'+products[indexCart].product.slug + '?back=cart'" class="image">
                                                        {{ products[indexCart].product.name }}
                                                        </router-link>
                                                    </p>
                                                    <p class="mb-0">
                                                        {{ products[indexCart].qty }} x 
                                                        <span v-if="products[indexCart].product.price_discount">
                                                            {{ products[indexCart].product.price_discount }} 
                                                            <del style="font-size: 12px; font-weight: normal;">{{ products[indexCart].product.price_rp }} </del>
                                                        </span>
                                                        <span v-else>
                                                            {{ products[indexCart].product.price_rp }} 
                                                        </span>
                                                        <b class="pull-right">Rp. {{ formatRibu(products[indexCart].product.price_final * products[indexCart].qty) }}</b>
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="card border-none-trl-mobile mb-4">
                        <div class="card-body p-0-mobile">
                            ...
                            <div class="comment-area-wrapper" v-else
                                v-if="user && data.length > 0 && products.length > 0"
...
export default {
    ...
    data() {
        return {
            data: [],
            products: this.$store.state.default.carts.product ?? [],
